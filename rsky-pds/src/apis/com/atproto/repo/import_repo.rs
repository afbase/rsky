...
use crate::repo::types::RecordWriteDescript;
    ...
    if let Ok(mut car) = read_car(incoming_car).await {
        ...
        for write in diff.writes {
            ...
            if let RecordWriteDescript::Delete(_) = write {
                ...
                let parsedRecord = parsed.record;
                let  _indexRecord = actor_store.record.index_record(
                    uri,
                    write.cid,
                    Ok(parsedRecord),
                    write.action(),
                ...
                // use rsky_lexicon::serialize::LexValue;
                let record_blobs = find_blob_refs(parsedRecord, 0);
...
pub fn find_blob_refs(val: &LexValue, layer: usize) -> Vec<BlobRef> {
    ...
    match val {
        ...
        LexValue::Blob(blob_ref) => vec![blob_ref.clone()],
        LexValue::Ipld(ipld_val) => {
            match ipld_val {
                IpldValue::Cid(_) | IpldValue::Bytes(_) => vec![],
                IpldValue::Array(arr) => {
                    arr.iter()
                        .flat_map(|item| find_blob_refs(&LexValue::Ipld(item.clone()), layer + 1))
                        .collect()
                },
